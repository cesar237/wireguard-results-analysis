...
function get_stat() {
    ...
    cat $file | grep "${chain} stats:" -A 1 | tail -1 | awk '{print $NF}'
}
...
function perf_stats() {
    ...
    file=out-stats.perf
    cache_miss=$(get_stat $file "cache-misses")
    cache_ops=$(get_stat $file "cache-references")
    context_switch=$(get_stat $file "cs")
    migrations=$(get_stat $file "migrations")
    faults=$(get_stat $file "faults")
    l1_miss=$(get_stat $file "L1-dcache-load-misses")
    l1_ops=$(get_stat $file "L1-dcache-loads")
    llc_miss=$(get_stat $file "LLC-load-misses")
    llc_ops=$(get_stat $file "LLC-loads")

    start=$(perf script | head -1 | tr -d ':' | awk '{ print $4 }' )
    end=$(perf script | grep ':' | tail -1 | tr -d ':' | awk '{ print $4 }' )

    echo "cache_miss,cache_ops,context_switch,migrations,faults,l1_miss,l1_ops,llc_miss,llc_ops,start,end"
    echo $cache_miss,$cache_ops,$context_switch,$migrations,$faults,$l1_miss,$l1_ops,$llc_miss,$llc_ops,$start,$end
...
function draw_flamegraph() {
    ...
    rm /tmp/out.folded
    stackcollapse-perf.pl $FILE > /tmp/out.folded
    flamegraph.pl /tmp/out.folded
    rm /tmp/out.folded
}